let g:leaderf#calibre#db = get(g:, 'leaderf#calibre#db', expand('~/Calibre Library/metadata.db'))
let g:leaderf#calibre#temp_dir = get(g:, 'leaderf#calibre#temp_dir', has('unix') ? '/tmp' : $HOME .. '/AppData/Local')
let g:leaderf#calibre#attr = get(g:, 'leaderf#calibre#attr', ['title', 'author', 'tag'])
let g:leaderf#calibre#default = 'view'

" load information of ebooks {{{ "
function! leaderf#calibre#sql(db, cmd) abort "{{{
	return split(system('echo .quit | sqlite3 -cmd ' .. shellescape(a:cmd) .. ' ' .. fnameescape(a:db)), "\n")
endfunction "}}}
...
function! leaderf#calibre#get_attr(infos, attr) abort "{{{
	...
	elseif a:attr ==# 'path'
		return fnamemodify(g:leaderf#calibre#db, ':h') .. '/' ..  a:infos[9] .. '/' .. a:infos[18] .. '.' .. tolower(a:infos[16])
	...
	elseif a:attr ==# 'tag'
		let l:tags = []
		for l:line in leaderf#calibre#get_tags(g:leaderf#calibre#db, a:infos[0])
...
function! leaderf#calibre#source(args) abort "{{{
	let l:lines = []
	let s:books = leaderf#calibre#get_books(g:leaderf#calibre#db)
	for l:book in s:books
		let l:infos = split(l:book, '|')
		let l:attrs = []
		for l:attr in ['id'] + g:leaderf#calibre#attr
...
function! leaderf#calibre#accept(line, args) abort "{{{
	...
	call leaderf#calibre#{g:leaderf#calibre#default}(l:id)
endfunction "}}}
...
function! leaderf#calibre#open(path) abort "{{{
	if has('win32')
		let l:cmd = '!start rundll32 url.dll,FileProtocolHandler ' .. shellescape(a:path)
	elseif has('mac')
		let l:cmd = 'open ' .. shellescape(a:path)
	elseif executable('xdg-open')
		let l:cmd = 'xdg-open ' .. shellescape(a:path)
	endif
	call system(l:cmd)
endfunction "}}}
...
function! leaderf#calibre#open_path(id) abort "{{{
	let l:path = fnameescape(fnamemodify(leaderf#calibre#id_to_path(a:id), ':h'))
...
function! leaderf#calibre#yank_path(id) abort "{{{
	...
	for l:reg in get(g:, 'leaderf#calibre#regs', ['0', '*', '+'])
		execute 'let @' .. l:reg '=' shellescape(l:path)
	endfor
endfunction "}}}
...
function! leaderf#calibre#edit_opf(id) abort "{{{
	let l:temp = g:leaderf#calibre#temp_dir .. '/' .. a:id .. '.opf'
	if leaderf#calibre#calibredb('show_metadata --as-opf ' .. a:id .. ' > ' .. l:temp)
		execute 'edit' l:temp
		let b:temp = l:temp
		let b:cmd = 'calibredb set_metadata ' .. a:id .. ' ' .. l:temp
		augroup leaderf_calibre "{{{
			autocmd!
			autocmd BufWritePost <buffer> call system(b:cmd)
			autocmd BufWritePost <buffer> call delete(b:temp)
		augroup END "}}}
	endif
endfunction "}}}
